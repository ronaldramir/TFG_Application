...
# Storytelling: tabs
# -------------------------------------------------------
tab_resumen, tab_geo, tab_mercado, tab_precio, tab_specs, tab_colores = st.tabs(
    ["🧾 Resumen", "🗺️ Geografía", "🏷️ Mercado", "💰 Precio", "🛠️ Specs", "🎨 Colores"]
)
...
with tab_geo:
    ...
    with st.container(border=True):
        ...
        if "provincia" not in fdf.columns or fdf["provincia"].dropna().empty:
            ...
            # Mapa: intenta cargar data/gadm41_CRI_1.json automáticamente
            geo_path = Path("data/gadm41_CRI_1.json")
            cr_geo = None
            if geo_path.exists():
                try:
                    cr_geo = json.loads(geo_path.read_text(encoding="utf-8"))
                    st.caption("GeoJSON cargado automáticamente desde: data/gadm41_CRI_1.json")
                except Exception as e:
                    st.warning(f"No pude leer data/gadm41_CRI_1.json (¿archivo corrupto?). Error: {e}")

            # Fallback a uploader si no existe o falló
            if cr_geo is None:
                st.markdown("**Mapa (opcional):** si no está el archivo en `/data`, subí un GeoJSON de provincias.")
                gj = st.file_uploader("GeoJSON de provincias (opcional)", type=["json", "geojson"])
                if gj is not None:
                    try:
                        cr_geo = load_geojson(gj)
                    except Exception as e:
                        st.warning(f"No se pudo leer el GeoJSON subido. Error: {e}")

            if cr_geo is not None:
                try:
                    # Normalización robusta para matching provincia <-> GeoJSON
                    prov_pct = prov_pct.copy()
                    ...
                    st.warning(f"No se pudo generar el mapa con el GeoJSON disponible. Error: {e}")


# =========================
# TAB: Mercado (marca/modelo + categóricas)
# =========================
with tab_mercado:

    # --- Participación marca (pie)
    with st.container(border=True):
    ...
    with st.container(border=True):
        ...
        if marca_col in fdf.columns and fdf[marca_col].notna().any():
            top_n_b = st.slider("Top N marcas (barras)", 5, 25, 12)

            counts = fdf[marca_col].astype(str).value_counts().head(top_n_b).reset_index()
            counts.columns = ["marca", "cantidad"]

            fig = px.bar(counts, x="marca", y="cantidad", title=f"Top {top_n_b} marcas")
            fig.update_layout(xaxis_title="Marca", yaxis_title="Cantidad de anuncios")
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.info(f"No hay datos suficientes en '{marca_col}'.")

    # --- Top modelos (barras horizontales)
    with st.container(border=True):
        st.header("🚗 Top 15 modelos más vendidos")

        if "modelo_norm" in fdf.columns:
            col_modelo = "modelo_norm"
        elif "modelo" in fdf.columns:
            col_modelo = "modelo"
        else:
            col_modelo = None

        if not col_modelo or fdf[col_modelo].dropna().empty:
            st.info("No encontré columna 'modelo'/'modelo_norm' con datos suficientes.")
        else:
            top_modelos = fdf[col_modelo].astype(str).value_counts().head(15).reset_index()
            top_modelos.columns = ["Modelo", "Cantidad"]

            fig_top_modelos = px.bar(
                top_modelos.sort_values("Cantidad"),
                x="Cantidad",
                y="Modelo",
                orientation="h",
                text="Cantidad",
                title="Top 15 Modelos Más Vendidos"
            )
            fig_top_modelos.update_layout(height=650)
            st.plotly_chart(fig_top_modelos, use_container_width=True)

    # --- Categóricas % (para contar la historia: preferencias)
    with st.container(border=True):
        st.header("📌 Preferencias del mercado (categóricas en %)")

        default_cols = [c for c in ["estilo", "combustible", "transmision", "estado"] if c in fdf.columns]
        if not default_cols:
            st.info("No encontré columnas típicas (estilo/combustible/transmision/estado) en este dataset.")
        else:
...
# =========================
# TAB: Precio (distribución + relaciones)
# =========================
with tab_precio:
...
# =========================
# TAB: Specs (cilindrada y otras numéricas)
# =========================
with tab_specs:
...
st.caption("EDA interactivo en Streamlit. Si algo no aparece, casi seguro es porque el CSV no trae la columna, no porque yo tenga ganas de sufrir.")
